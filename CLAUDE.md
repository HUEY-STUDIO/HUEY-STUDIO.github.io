...
"cardnews": [
  {
    ...
    "photo": "https://images.unsplash.com/...",   // 선택. 있으면 브랜드 그래픽 대신 이 사진을 배경으로 쓴다(하단 스크림은 그대로)
    "photo_credit": "Jane Doe",                   // 선택. photo가 있을 때만 카드 우하단에 작게 "Photo: Jane Doe / Unsplash" 표기
    "slides": [
...
- 배경은 기본적으로 `cardnews.py`가 생성하는 브랜드 그래픽(블랙·딥레드·포인트레드 추상
  패턴)이다 — 매체(언론사) 사진을 카드뉴스 배경으로 캡처해 쓰지 않는다(핫링크가 아니라 이미지를
  그대로 복제해 재배포하는 셈이라 저작권 리스크가 사이트 임베딩보다 훨씬 크다).
  대신 Unsplash 등 재배포 가능한 스톡 사진은 `photo` 필드로 넣을 수 있다 — 이때는 브랜드
  그래픽 대신 그 사진이 배경이 되고(하단 스크림·텍스트는 동일), `photo_credit`을 함께 채우면
  카드 우하단에 작게 사진작가 크레딧이 표기된다.
